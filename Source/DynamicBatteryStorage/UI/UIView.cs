...
namespace DynamicBatteryStorage.UI
{
  public class UIView: UIWidget
  {
    ...
    protected virtual void DrawFooterPanel()
    {
      ...
      if (GUILayout.Button(intermittentToggle, UIHost.GUIResources.GetStyle("radio_text_button")))
        ToggleHandlerSimulationStates(false);
      if (GUILayout.Button(intermittentOn, UIHost.GUIResources.GetStyle("radio_text_button")))
        SetHandlerSimulationStates(true, false);
      if (GUILayout.Button(intermittentOff, UIHost.GUIResources.GetStyle("radio_text_button")))
      ...
      if (GUILayout.Button(intermittentOff, UIHost.GUIResources.GetStyle("radio_text_button")))
        SetHandlerSimulationStates(false, true);
      GUILayout.EndHorizontal();
      GUILayout.EndVertical();
    ...
    protected virtual void DrawDetailPanel()
    {
      ...
      if (GUILayout.Button(" ", UIHost.GUIResources.GetStyle("negative_button"), GUILayout.Width(col_width)))
      {
        ...
        {
          consumerCategoryUIItems[categoryNames[i]].SetDirty();
        }
      }
      ...
      if (showDetails)
      {
        scrollPosition = GUILayout.BeginScrollView(scrollPosition, GUILayout.Width(col_width*2f), GUILayout.Height(Mathf.Min(scrollHeight, maxScrollHeight)));
